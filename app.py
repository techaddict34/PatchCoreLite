...
def load_model():
    ...
    # capture spatial feature maps for the heatmap (before flattening em)
    spatial  = []
...
def generate_heatmap(captured_features, memory_bank, img_display_rgb):
    """
    For every spatial patch position, compute its nearest neighbor dist
    in the memory bank, reshape to a 2D map, upsample to image size, and
    ...
    combined  = torch.cat([f2_pooled, f3_up], dim=1)

    # Flatten to patches, like in preprocessing.py
    patches = combined.squeeze(0).reshape(384, -1).T   
    chunk   = 256
    min_dists = []
    for i in range(0, patches.shape[0], chunk):
        d = torch.cdist(patches[i:i+chunk], memory_bank, p=2)
        min_dists.append(d.min(dim=1).values)
    patch_scores = torch.cat(min_dists).reshape(H, W).cpu().numpy()  

    # Normalise to [0, 1], can't get out of bounds
    ...
    # Upsample back to 224×224 (from 28x28)
...
if uploaded:
    ...
    padded   = resize_with_pad(raw_rgb, needed_size=224) 
    ...
    with col1:
        ...
        st.image(padded, use_column_width="always")
    ...
    with col2:
        ...
        st.image(cv2.cvtColor(heat_only, cv2.COLOR_BGR2RGB), use_column_width="always")

    with col3:
        st.markdown("**Overlay**")
        st.image(blended, use_column_width="always")
